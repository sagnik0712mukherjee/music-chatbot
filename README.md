...
- Per-chat model selection using friendly labels like "The Thinker" and "The Maestro".
...
The app exposes friendly model names while mapping them to the local Ollama models currently in use:

- `The Thinker` — `qwen3.6:35b-a3b`
- `The Maestro` — `deepseek-r1:14b`

> Note: This app now uses only these pulled Ollama models for consistency and best local performance.
...
If a pull fails due to networking or TLS issues, check your local connection and retry. Use `ollama ls` to confirm the pulled models and then refresh the app.
...
- Adjust the `Innovation` slider for a controlled blend of creativity and accuracy. The app limits randomness so it prefers verified music answers over invented details.
...
- If the UI is slow, try switching to a smaller model like `The Maestro` (`deepseek-r1:14b`) instead of larger models.
